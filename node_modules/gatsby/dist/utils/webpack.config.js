  ...
  var _ref = (0, _asyncToGenerator2.default)(function* (program, directory, suppliedStage, webpackPort = 1500) {
    ...
    function getPlugins() {
      ...
      switch (stage) {
        case `develop`:
          configPlugins = configPlugins.concat([plugins.hotModuleReplacement(), plugins.noEmitOnErrors(), new FriendlyErrorsWebpackPlugin({
            ...
            compilationSuccessInfo: {
              messages: [`You can now view your site in the browser running at ${program.ssl ? `https` : `http`}://${program.host}:${program.port}`, `Your graphql debugger is running at ${program.ssl ? `https` : `http`}://${program.host}:${program.port}/___graphql`]
            }
          })]);
          break;
        ...
        case `build-javascript`:
          {
            configPlugins = configPlugins.concat([plugins.extractText(), // Minify Javascript.
            plugins.uglify({
              uglifyOptions: {
                compress: {
                  drop_console: false
